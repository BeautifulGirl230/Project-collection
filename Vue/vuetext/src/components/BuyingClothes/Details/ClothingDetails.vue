<template>
  <div>
    ...
    <mt-header fixed title="商品详情">
      <router-link to="/purchase/clothing" slot="left">
        ...
        methods:{
          log(){
            ...
            DetailsImg(){
                this.$forceUpdate();
              //获救json里的值
                let localJSON = '/api/static/Snake.json';
                this.$http.get(localJSON).then( response => {
                    this.detailsimg = response.body.clothing;
                    //过滤数据
